...
uint16_t check_debug_Relay3=0;
uint16_t check_debug_Relay2=0;
uint16_t check_debug_Relay1=0;
...
//	}
	...
	if(HAL_GPIO_ReadPin(GPIO_RELAY, PIN_RELAY3)==1 && check_debug_Relay3 ==0)
	{
		HAL_UART_Transmit(uart, (uint8_t *)("Relay3_ON"), (uint16_t)strlen("Relay3_ON"),1000);
		HAL_UART_Transmit(uart,(uint8_t *)"\r\n",(uint16_t)strlen("\r\n"),1000);	
		check_debug_Relay3=1;
	}
	if(HAL_GPIO_ReadPin(GPIO_RELAY, PIN_RELAY3)==0 && check_debug_Relay3 ==1)
	{
		HAL_UART_Transmit(uart, (uint8_t *)("Relay3_OFF"), (uint16_t)strlen("Relay3_OFF"),1000);
		HAL_UART_Transmit(uart,(uint8_t *)"\r\n",(uint16_t)strlen("\r\n"),1000);	
		check_debug_Relay3=0;
	}
	
	if(HAL_GPIO_ReadPin(GPIO_RELAY, PIN_RELAY2)==1 && check_debug_Relay2 ==0)
	{
		HAL_UART_Transmit(uart, (uint8_t *)("Relay2_ON"), (uint16_t)strlen("Relay2_ON"),1000);
		HAL_UART_Transmit(uart,(uint8_t *)"\r\n",(uint16_t)strlen("\r\n"),1000);	
		check_debug_Relay2=1;
	}
	if(HAL_GPIO_ReadPin(GPIO_RELAY, PIN_RELAY2)==0 && check_debug_Relay2 ==1)
	{
		HAL_UART_Transmit(uart, (uint8_t *)("Relay2_OFF"), (uint16_t)strlen("Relay2_OFF"),1000);
		HAL_UART_Transmit(uart,(uint8_t *)"\r\n",(uint16_t)strlen("\r\n"),1000);	
		check_debug_Relay2=0;
	}
	
	if(HAL_GPIO_ReadPin(GPIO_RELAY, PIN_RELAY1)==1 && check_debug_Relay1 ==0)
	{
		HAL_UART_Transmit(uart, (uint8_t *)("Relay1_ON"), (uint16_t)strlen("Relay1_ON"),1000);
		HAL_UART_Transmit(uart,(uint8_t *)"\r\n",(uint16_t)strlen("\r\n"),1000);	
		check_debug_Relay1=1;
	}
	if(HAL_GPIO_ReadPin(GPIO_RELAY, PIN_RELAY1)==0 && check_debug_Relay1 ==1)
	{
		HAL_UART_Transmit(uart, (uint8_t *)("Relay1_OFF"), (uint16_t)strlen("Relay1_OFF"),1000);
		HAL_UART_Transmit(uart,(uint8_t *)"\r\n",(uint16_t)strlen("\r\n"),1000);	
		check_debug_Relay1=0;
	}
}
