        ...
        if (sExFlash.sHWrite.Pending_u8 != true)
        {
            if (qGet_Number_Items (&qFlashWrite) != 0)
            {
                ...
                if (sExFlash.Status_u8 == true)
                {
                    ...
                    if ( sExFlash.pMem_Get_Addr != NULL )
                    {
                        sExFlash.sHWrite.Addr_u32 = sExFlash.pMem_Get_Addr(qFlashWriteTemp.TypeData_u8, __MEM_WRITE);
        ...
        if (sExFlash.sHRead.Pending_u8 != true)
        {
            ...
            {   
                ...
                if (sExFlash.Status_u8 == true)
                {
                    ...
                    if ( sExFlash.pMem_Get_Addr != NULL )
                    {
                        sExFlash.sHRead.Addr_u32 = sExFlash.pMem_Get_Addr(qFlashReadTemp.TypeData_u8, __MEM_READ);
                        //Neu la 1 queue ghi vao 1 dia chi cu the: k phai ban tin data: lay dia chi trong queue
...
static uint8_t _Cb_Flash_Cmd_Failure(void)
{
    ...
    */
    ...
    {
        UTIL_Printf_Str( DBLEVEL_M, "u_ex_flash: skip item queue write!\r\n");
...
static uint8_t _Cb_Read_Check_write(void)
{
    ...
    */
    ...
    for (uint16_t i = 0; i < sExFlash.sHWrite.Length_u16; i++)
    {
        ...
        {
            UTIL_Printf_Str (DBLEVEL_M, "u_ex_flash: read again compare error!\r\n");
...
uint8_t eFlash_Test_Write (void)
{
    ...
    if (sExFlash.sHWrite.Pending_u8 != true)
    {
        ...
        eFlash_Erase_Sector(FLASH_ADDR_TEST);
    #endif
        ...
        sExFlash.sHWrite.TypeData_u8  = _MEM_DATA_OTHER;
...
uint8_t eFlash_Check_Pos_PartA (sRecordMemoryManager *sRec)
{
    ...
    #if (FLASH_MANAGE_TYPE == TYPE_PART_AB)
        //xoa sector de lan sau check
        eFlash_Erase_Sector(sRec->AddAStart_u32);
    #endif
    ...
    if (sExFlashInit.Status_u8 == true)
    {
        ...
        sRec->IndexSave_u16 = MEM_CACUL_INDEX (sRec->AddA_u32, sRec->AddAStart_u32, sRec->SizeRecord_u16);
        sExFlashInit.Status_u8 = false; 
    ...
    {          
        ...
        sExFlash.sHRead.TypeRq_u8          = _MEM_READ_INIT;
        sExFlash.sHRead.TypeData_u8        = _MEM_DATA_OTHER;
...
uint8_t eFlash_Check_Pos_PartB (sRecordMemoryManager *sRec)
{
    ...
    if (sExFlashInit.Status_u8 == true)
    {
        ...
        #if (FLASH_MANAGE_TYPE == TYPE_PART_AB)
            //xoa sector de lan sau check
            eFlash_Erase_Sector(sRec->AddB_u32);
        #endif
        }
        ...
        sRec->IndexSend_u16 = MEM_CACUL_INDEX (sRec->AddB_u32, sRec->AddBStart_u32, sRec->SizeRecord_u16);
